...
- **Compliance score**: `ComplianceService.getComplianceStats` averages each project’s `projectControls.progress` to produce an `overallScore` and per-project `progress`. This score powers the customer dashboard and ensures 0-100 values even when controls are missing.
