...
## 8. Memory System (`backend/src/services/memory.service.ts`)

- **Long-term storage**: The system uses the `AgentMemory` model to persist core facts, daily logs, and evidence extracts across conversations.
- **Memory Types**:
  * `core_fact`: Permanent knowledge about the project or customer.
  * `daily_log`: Summaries of recent activity or changes.
  * `evidence_extract`: Key findings or data points extracted from analyzed evidence.
- **Retrieval**: Memories are scoped by `customerId` and optionally `projectId`. They can be queried with `memoryType` and `category` filters.
- **Dreaming Cycle**: The `DreamingService` (triggered via `POST /api/admin/ai-compliance/dreaming`) performs periodic consolidation, merging short-term activity into core memories.
- **AI Tooling**: The AI agent uses `get_project_memories` to retrieve relevant context before answering complex compliance questions.
